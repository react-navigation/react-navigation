...
it('will not attempt to navigate beyond whatever browser history it is possible to know about, using browser history', () => {
  jest.useFakeTimers();
  const windowGoSpy = jest.spyOn(window.history, 'go');
  ...
  window.history.go(-1);
  jest.runAllTimers();
  expect(windowGoSpy).toHaveBeenCalledTimes(5);
  expect(history.index).toBe(0);

  // When we navigate forward once then we should see our index change
  window.history.go(1);
  jest.runAllTimers();
  expect(windowGoSpy).toHaveBeenCalledTimes(6);
  expect(history.index).toBe(1);
  ...
  jest.runAllTimers();
  expect(history.index).toBe(0);
  expect(windowGoSpy).toHaveBeenCalledTimes(7);
