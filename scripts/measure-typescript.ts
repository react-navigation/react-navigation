...
void Use${name};`;
  ...
  const linkConsumers = screens
    .filter(({ level }) => level === 0)
    .map(({ name, index }) => {
      const param = paramName(0, index);

      return `function UseLink${name}() {
  const props = useLinkProps<RootParamList>({
    screen: '${name}',
    params: { ${param}: 1 },
  });
  void props;
  ...
  return (
    <Link<RootParamList> screen="${name}" params={{ ${param}: 1 }}>
      ${name}
    </Link>
  );
}
