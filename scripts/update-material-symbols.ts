...
for (const variant of VARIANTS) {
  process.stdout.write(`\nFetching font URL for ${variant}...`);

  const cssResponse = await fetch(
    `https://fonts.googleapis.com/css2?family=Material+Symbols+${variant}`,
    {
      headers: {
        'User-Agent':
  ...
  if (!cssResponse.ok) {
    throw new Error(
      `Failed to fetch CSS for ${variant}: ${cssResponse.statusText}`
    );
  }
  ...
  if (!fontUrlMatch) {
    throw new Error(`Could not find font URL in CSS for ${variant}`);
  }
  ...
  const ttfDest = new URL(`MaterialSymbols${variant}.ttf`, assets);
  const ttfBuffer = await download(fontUrl, ttfDest);
  ...
  hashes.set(variant.toLowerCase(), hash);
...
export const MATERIAL_SYMBOL_FONT_HASHES = {
  outlined: '${hashes.get('outlined')}',
  rounded: '${hashes.get('rounded')}',
  sharp: '${hashes.get('sharp')}',
} as const;
