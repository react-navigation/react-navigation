...
class PageWithSidebar extends Component {
  render() {
    ...
    let prevName = prevAction &&
      getConfig(router, state, prevAction, 'linkName');
    let nextName = nextAction &&
      getConfig(router, state, nextAction, 'linkName');
        ...
        <div className="inner-page-body">
                      ...
                      <div>
                        {route.routes &&
                          route.routes.map((childRoute, childI) => {
                            ...
                            const secondOptions = DocComponent.router &&
                              DocComponent.router.getScreenOptions(
                                secondChildNavigation,
                                {},
                              );
                            const routerLinkName = secondOptions &&
                              secondOptions.linkName;
                            const linkName = routerLinkName ||
                              childRoute.routeName;
                            return (
                              <Link
                                to={childRoute.routeName}
                                className={
                                  `pt-menu-item page ${isChildActive ? 'active' : ''}`
                                }
                                key={childI}
                              >
                                {linkName}
          ...
          <div className="main-section">
            ...
            {state.routeName === 'Docs' &&
              <Link
                href={
                  `https://github.com/react-community/react-navigation/tree/master/docs/${docPath}`
                }
                className="editLink"
              >
                {' '}Edit on GitHub
