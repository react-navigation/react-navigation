...
test('goes back to the proper history entry after popping nested stack', async ({
  page,
}) => {
...
test('rolls back browser history when browser back is prevented', async ({
  browserName,
  page,
}) => {
  await page.goto('/');
...
test('restores previous params on browser back after pushing params', async ({
  page,
}) => {
...
test('syncs browser history when switching tabs with fullHistory back behavior', async ({
  ...
  await page.goto('/bottom-tabs-full-history');

  await expect(page).toHaveURL('/bottom-tabs-full-history/first');
  ...
  await expect(page).toHaveURL('/bottom-tabs-full-history/second/1');
  ...
  await expect(page).toHaveURL('/bottom-tabs-full-history/third/2');
...
test('restores screens on browser back and forward after page reload', async ({
  ...
  // Avoid `page.reload()` since it adds an extra history entry on Firefox
...
test('adds and removes browser history entry when drawer is opened and closed', async ({
  page,
}) => {
  await page.goto('/');
  ...
  const overlay = page.getByRole('button', { name: 'Close drawer' });

  await page
    .getByLabel('Show navigation menu')
    .filter({ visible: true })
    .click();
