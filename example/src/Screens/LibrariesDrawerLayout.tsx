...
export function LibrariesDrawerLayout(_: StaticScreenProps<Params>) {
  const { showActionSheetWithOptions } = useActionSheet();
  const { colors } = useTheme();
  const { direction } = useLocale();

  const navigation = useNavigation('LibrariesDrawerLayout');
  const route = useRoute('LibrariesDrawerLayout');
...
LibrariesDrawerLayout.title = 'Libraries - Drawer Layout';
LibrariesDrawerLayout.linking = linking;
LibrariesDrawerLayout.options = {
  headerShown: true,
};
