  ...
  ({ item, width }: { item: Dino; width?: number }) => {
    ...
    return (
      <Pressable
        ...
        onPress={() => navigation.navigate('DinoDetail', { dinoId: item.id })}
      >
...
const DinoDetailScreen = ({ route }: StaticScreenProps<{ dinoId: string }>) => {
  const { colors, fonts } = useTheme();
  const navigation = useNavigation('DinoDetail');
  const dino =
    DINOS.find((candidate) => candidate.id === route.params.dinoId) ?? DINOS[0];
  ...
  return (
    ...
    >
      <Pressable
        ...
        onPress={() => navigation.navigate('ImageViewer', { dinoId: dino.id })}
      >
        <Image
          source={dino.image}
          style={styles.fillImage}
...
const ImageViewerScreen = ({
  route,
}: StaticScreenProps<{ dinoId: string }>) => {
...
const styles = StyleSheet.create({
  ...
  cardImage: {
    aspectRatio: 1,
    // borderRadius: BORDER_RADIUS_L - SPACING_M,
    borderCurve: 'continuous',
    overflow: 'hidden',
  },
  fillImage: {
